...
-- This database will remember who already claimed their key
local AppleHubKeyStore = DataStoreService:GetDataStore("AppleHubKeyClaimedStore")

-- The secret string "AppleHub" in lowercase hexadecimal
local APPLE_HUB_KEY = "4170706c65487562"

-- Function to handle the math game when a player joins
game.Players.PlayerAdded:Connect(function(player)
	-- 1. Check if the player has already claimed the key in the past
	local success, alreadyClaimed = pcall(function()
		return AppleHubKeyStore:GetAsync(tostring(player.UserId))
	end)
	
	if alreadyClaimed then
		print(player.Name .. " has already claimed their key. Access Denied.")
		-- You can kick them or show a GUI saying they already did it
		player:Kick("You have already completed this challenge and used your key.")
		return
	end
	
	print("=== RANDOMIZED APPLE HUB KEY CHALLENGE FOR " .. player.Name:upper() .. " ===")
	print("Solve these 3 math problems to get your key.")
	
	-- --- QUESTION 1 (Easy) ---
	local a1 = math.random(5, 20)
	local b1 = math.random(2, 5)
	local c1 = math.random(2, 6)
	local q1_ans = a1 + (b1 * c1)
	
	-- For this example, we print questions to the Server output. 
	-- In a real game, you would send this to a TextBox GUI on the player's screen.
	print("Question 1: What is " .. a1 .. " + (" .. b1 .. " * " .. c1 .. ")?")
	
	-- Simulated user answer (Replace this logic with your UI input)
	local playerAnswer1 = q1_ans -- Simulating a correct answer for demonstration
	
	if playerAnswer1 ~= q1_ans then
		player:Kick("Incorrect answer! Try again by rejoining.")
		return
	end
	
	-- --- SUCCESS: Save to Database and Give Key ---
	local saveSuccess, err = pcall(function()
		AppleHubKeyStore:SetAsync(tostring(player.UserId), true)
	end)
	
	if saveSuccess then
		print("SUCCESS! Player passed. Your Apple Hub Key is: " .. APPLE_HUB_KEY)
		-- Send the key to the player's UI here
	else
